...
export const getAssociationRequests = async (req: Request, res: Response) => {
    try {
        ...
        const associationRequestUsersTemp: Array<IAssociationRequestUsersRes> =
            []
        for (let i = 0; i < associationRequests.length; i++) {
            // get all the users that have requested an association where you, the user were the recipient
            ...
            associationRequestUsersTemp.push({
                ...
                associationRequestUsername: associationRequestUsers[i].username,
...
export const actionOnAssociationRequest = async (
    ...
    try {
        ...
        const recipient = await userModel.findOne({
            username: req.user.user_id, // checking if the recipient is the one that is authenticated. Only recipients can take action so its ok
        })
